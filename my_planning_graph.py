...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            n_to_add = PgNode_a(action)
            if n_to_add.prenodes.issubset(self.s_levels[level]):
                action_node.append(n_to_add)
            # then we must link to the rest of the graph
            #create node
            for n in self.s_levels[level] :
                n.children.add(n_to_add) 
                n_to_add.parents.add(n) 
    ...
    def add_literal_level(self, level):
        ...
        lit_nodes = set()
        for node_action in self.a_levels[level - 1]:
            for lit_node in node_action.effnodes:
                lit_nodes.add(lit_node)
                lit_node.parents.add(node_action)
                node_action.children.add(lit_node)
        self.s_levels.append(lit_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #check action of first
        l1= node_a1.action.effect_add
        l2= node_a1.action.effect_rem
        l3= node_a2.action.effect_add
        l4= node_a2.action.effect_rem
        #check for inconsistent effect between rem and add
        for a in l1:
            if a in l4:
                return True
        for a in l2:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        l1= node_a1.action.effect_add
        l2= node_a1.action.precond_neg
        l3= node_a2.action.effect_add
        l4= node_a2.action.precond_neg
        l5= node_a1.action.effect_rem
        l6= node_a1.action.precond_pos
        l7= node_a2.action.effect_rem
        l8= node_a2.action.precond_pos
        # check for interference in each effect/precond
        for a in l1:
            if a in l4:
                return True
        for a in l2:
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        #can be done with pos and state equality as suggested
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        #for each goal
        for gl in self.problem.goal:
            lvl=0 
            found = False
            #for each levels
            for lvl_s in self.s_levels:
                lvl= lvl+1
                #for each state
                for st in lvl_s:
                    if gl == expr(st.symbol) and st.is_pos:
                        found = True
                        level_sum = lvl+level_sum
                        break
                if found:
                    break
        return level_sum-2
